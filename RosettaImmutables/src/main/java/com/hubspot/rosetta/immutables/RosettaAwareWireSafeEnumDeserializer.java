...
public class RosettaAwareWireSafeEnumDeserializer
  ...
  private static <T extends Enum<T>> JsonDeserializer<?> deserializerFor(
    Class<?> rawType
  ) {
    return DESERIALIZER_CACHE.computeIfAbsent(
      rawType,
      ignored -> newDeserializer((Class<T>) rawType)
    );
  }
  ...
  private static Class<?> extractEnumType(DeserializationContext context)
    throws JsonMappingException {
